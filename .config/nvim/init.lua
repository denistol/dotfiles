...
vim.opt.tabstop = 2 -- Tab width (2 spaces instead of 4)
vim.opt.softtabstop = 2 -- Tab count while editing
...
vim.opt.wrap = false -- Disable line wrap by default (nowrap)

-- Treesitter smart folding (auto folds on functions, methods, classes)
vim.opt.foldmethod = "expr"
vim.opt.foldexpr = "v:lua.vim.treesitter.foldexpr()"
vim.opt.foldlevel = 99 -- Open all files fully expanded by default
vim.opt.foldlevelstart = 99
vim.opt.foldnestmax = 5 -- Maximum fold nesting depth

-- Clean VS Code-like fold appearance (shows method name first, line count at the end)
function _G.custom_foldtext()
	local start_line = vim.fn.getline(vim.v.foldstart)
	local end_line = vim.fn.trim(vim.fn.getline(vim.v.foldend))
	local line_count = vim.v.foldend - vim.v.foldstart + 1

	local clean_start = vim.fn.substitute(start_line, [[\s\+$]], "", "")
	local closing = ""
	if end_line == "}" or end_line == "};" or end_line == "]" or end_line == "];" then
		closing = " " .. end_line
	end

	return clean_start .. " ⋯" .. closing .. "  (" .. line_count .. " lines)"
end

vim.opt.foldtext = "v:lua.custom_foldtext()"

-- Russian layout support for Normal mode commands (hjkl, dd, w, b, etc.)
local function escape_langmap(str)
	local escape_chars = [[;,."|\]]
	return vim.fn.escape(str, escape_chars)
end
local en_keys = [[`qwertyuiop[]asdfghjkl;'zxcvbnm]]
local ru_keys = [[ёйцукенгшщзхъфывапролджэячсмить]]
local en_shift_keys = [[~QWERTYUIOP{}ASDFGHJKL:"ZXCVBNM<>]]
local ru_shift_keys = [[ЁЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮ]]

vim.opt.langmap = vim.fn.join({
	escape_langmap(ru_shift_keys) .. ";" .. escape_langmap(en_shift_keys),
	escape_langmap(ru_keys) .. ";" .. escape_langmap(en_keys),
}, ",")
...
-- -----------------------------------------------------------------------------
require("lazy").setup({
	-- Langmapper: Russian layout support for all keymaps, plugins, and Ctrl sequences
	{
	...
	-- Git Diff Indicators in Sign Column (gitsigns)
	{
		...
		opts = {
			...
			on_attach = function(bufnr)
				...
				end, { desc = "Git blame line (Space+hb)" })
	...
	-- Diffview (Full project diff & side-by-side comparison)
	{
		...
		config = function()
			...
			vim.keymap.set(
				...
				{ desc = "Git File History (Space+gh)" }
			)
		end,
	},
	...
	-- Telescope & Dependencies
	{
		...
		config = function()
			...
			telescope.setup({
				...
				pickers = {
					find_files = {
						hidden = true, -- искать скрытые файлы (.env, .zshrc и т.д.)
						-- no_ignore = true, -- раскомментируйте, если нужно искать и в файлах из .gitignore
					},
					live_grep = {
						additional_args = function()
							return { "--hidden" } -- искать текст в скрытых файлах
						end,
					},
				},
			})
		...
		config = function()
			...
			local on_attach = function(_, bufnr)
				...
				vim.keymap.set(
					...
					"<leader>ca",
					trigger_code_action,
					vim.tbl_extend("force", opts, { desc = "Code actions (Space+ca)" })
				)
				vim.keymap.set(
					{ "n", "v" },
					"<leader>.",
					trigger_code_action,
					vim.tbl_extend("force", opts, { desc = "Code actions (Space+.)" })
				)
				vim.keymap.set(
					{ "n", "v", "i" },
					"<C-.>",
					trigger_code_action,
					vim.tbl_extend("force", opts, { desc = "Code actions (Ctrl+.)" })
				)
				vim.keymap.set(
					{ "n", "v" },
					"<A-CR>",
					trigger_code_action,
					vim.tbl_extend("force", opts, { desc = "Code actions (Alt+Enter)" })
				)
				vim.keymap.set(
					"n",
					"<leader>d",
